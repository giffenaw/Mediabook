class User < ApplicationRecord
  ...
  def pending_friend_requests_to
    self.friendships.where(state: "pending").map(&:user)
  end
  
  def friendship_status(user_2)
    friendship = Friendship.where(user_id: [self.id, user_2.id], friend_id: [self.id, user_2.id])
    if friendship.any? == false # if no friendship exists
      return "not friends"
    elsif friendship.first.state == "active" # if friendship exists and status is active
      return "friends"
    elsif friendship.first.user == self # if we have requested friendship
      return "pending"
    elsif friendship.first.friend == self # if they have requested friendship
      return "requested"
    end
  end
